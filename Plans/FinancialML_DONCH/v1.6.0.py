...
def init(utilities):
	...
	weights_path = os.path.join('\\'.join(__file__.split('/')[:-1]), plan_name+'_weights', '{}.json'.format(VARIABLES['plan']))
	with open(weights_path, 'r') as f:
		info = json.load(f)
		weights = [np.array(i, np.float32) for i in info['weights'][:2]]
		biases = [np.array(i, np.float32) for i in info['weights'][2:]]
		mean = info['mean']
		std = info['std']
...
def onNewBar(chart):
	...
	data = np.array(chart.getAskOHLC(utils, 0, 500), dtype=np.float32)
	inputs = getInputs(data[:,1], data[:,2], 4, 10)
