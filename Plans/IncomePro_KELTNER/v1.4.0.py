...
def runSequence():
	...
	if c_direction == Direction.LONG:
		...
		if isPreEntryConfirmed(c_direction, reverse=True):
			if VARIABLES['ct_direction'] in (0,1):
		...
		if isPreEntryConfirmed(c_direction, reverse=True):
			if VARIABLES['ct_direction'] in (0,2):
...
def adEntryOne(trigger, blocked=False):
	...
	if trigger and c_direction == trigger.direction and not blocked:
		...
		if trigger.ad_entry_one_state.value > AdEntryOneState.ONE.value and isCloseABLOneOut(trigger.direction):
			trigger.ad_entry_one_state = AdEntryOneState.TWO
			return
...
def isMacdRetConf(direction, reverse=False):
	hist = macd.getCurrent(utils, chart)[2]
...
def confirmation(trigger, entry_type, reverse=False):
	...
	if isPosInDir(trigger.direction) and len(utils.positions) >= VARIABLES['max_positions']:
		pending_entry = None
		return False
