...
class AdEntryOneAState(Enum):
	ONE = 1
	TWO = 2
	THREE = 3
	COMPLETE = 4

class AdEntryOneBState(Enum):
	ONE = 1
	TWO = 2
	THREE = 3
	COMPLETE = 4
...
class Trigger(dict):
	...
	def __init__(self, direction):
		...
		self.ad_entry_one_a_state = AdEntryOneAState.ONE
		self.ad_entry_one_b_state = AdEntryOneBState.ONE
...
def runSequence():
	...
	if c_direction == Direction.LONG:
		...
		if isPreEntryConfirmed(c_direction, reverse=True):
			if adEntryOneA(long_trigger, blocked): blocked = True
		if isPreEntryConfirmed(c_direction, reverse=True):
			if adEntryOneB(long_trigger, blocked): blocked = True
		...
		if isPreEntryConfirmed(c_direction, reverse=True):
			if adEntryOneA(short_trigger, blocked): blocked = True
		if isPreEntryConfirmed(c_direction, reverse=True):
			if adEntryOneB(short_trigger, blocked): blocked = True
		...
		if trigger.pre_t_entry_state == PreTEntryState.ONE:
			...
			if not isKeltOutside() and preEntryConfirmation(trigger.direction):
				...
				if trigger.direction == Direction.LONG:
					short_trigger.ad_entry_one_a_state = AdEntryOneAState.ONE
					short_trigger.ad_entry_one_b_state = AdEntryOneBState.ONE
					short_trigger.ad_entry_two_state = AdEntryTwoState.ONE
				else:
					long_trigger.ad_entry_one_a_state = AdEntryOneAState.ONE
					long_trigger.ad_entry_one_b_state = AdEntryOneBState.ONE
	...
	if direction != c_direction:
		...
		if reverse:
			if direction == Direction.LONG:
				...
				short_trigger.ad_entry_one_a_state = AdEntryOneAState.TWO
				short_trigger.ad_entry_one_b_state = AdEntryOneBState.TWO
				short_trigger.ad_entry_two_state = AdEntryTwoState.TWO
			else:
				...
				long_trigger.ad_entry_one_a_state = AdEntryOneAState.TWO
				long_trigger.ad_entry_one_b_state = AdEntryOneBState.TWO
				long_trigger.ad_entry_two_state = AdEntryTwoState.TWO
		else:	
			if direction == Direction.LONG:
				c_direction = Direction.LONG
...
def adEntryOneA(trigger, blocked=False):

	if trigger and c_direction == trigger.direction and not blocked:
		if trigger.ad_entry_one_a_state == AdEntryOneAState.ONE:
			if (
				isRetHitKelt(trigger.direction) or 
				isRetHitLThree(trigger.direction)
			):
				trigger.ad_entry_one_a_state = AdEntryOneAState.TWO
				return

		elif trigger.ad_entry_one_a_state == AdEntryOneAState.TWO:
			if isCloseABKSma(trigger.direction, reverse=True):
				trigger.ad_entry_one_a_state = AdEntryOneAState.THREE
				return

		elif trigger.ad_entry_one_a_state == AdEntryOneAState.THREE:
			if adEntryOneAConfirmation(trigger.direction) and not isKeltOutside():
				trigger.ad_entry_one_a_state = AdEntryOneAState.ONE
				trigger.ad_entry_one_b_state = AdEntryOneBState.ONE

				resetPreEntry(trigger.direction, reverse=True)
				confirmation(trigger, EntryType.AdOneAEntry)
				return True

		if trigger.ad_entry_one_a_state.value > AdEntryOneAState.ONE.value and isCloseABLOneOut(trigger.direction):
			trigger.ad_entry_one_a_state = AdEntryOneAState.TWO
			return

def adEntryOneB(trigger, blocked=False):

	if trigger and c_direction == trigger.direction and not blocked:
		if trigger.ad_entry_one_b_state == AdEntryOneBState.ONE:
			if (
				isRetHitKelt(trigger.direction) or 
				isRetHitLThree(trigger.direction)
			):
				trigger.ad_entry_one_b_state = AdEntryOneBState.TWO
				return

		elif trigger.ad_entry_one_b_state == AdEntryOneBState.TWO:
			if isRetHitKSma(trigger.direction):
				trigger.ad_entry_one_b_state = AdEntryOneBState.THREE
				return adEntryOneB(trigger, blocked)

		elif trigger.ad_entry_one_b_state == AdEntryOneBState.THREE:
			if adEntryOneBConfirmation(trigger.direction) and not isKeltOutside():
				trigger.ad_entry_one_a_state = AdEntryOneAState.ONE
				trigger.ad_entry_one_b_state = AdEntryOneBState.ONE

				resetPreEntry(trigger.direction, reverse=True)
				confirmation(trigger, EntryType.AdOneBEntry)
				return True

		if trigger.ad_entry_one_b_state.value > AdEntryOneBState.ONE.value and isCloseABSma(trigger.direction):
			trigger.ad_entry_one_b_state = AdEntryOneBState.TWO
			return

def adEntryOneAConfirmation(direction):
	if utils.plan_state.value in (1,4):
		utils.log('adEntryOneAConfirmation', 'Ad Entry One Conf: {0} {1} {2} {3}'.format(
...
def adEntryOneBConfirmation(direction):
	if utils.plan_state.value in (1,4):
		utils.log('adEntryBOneConfirmation', 'Ad Entry One Conf: {0} {1} {2} {3}'.format(
	...
	if trigger and c_direction != trigger.direction and not blocked:
		...
		if trigger.ct_entry_state == CTEntryState.ONE:
			...
			if ctReverseEntryConfirmation(trigger.direction):
				...
				if not isPosInDirection(trigger.direction, reverse=True):
...
def ctEntryConfirmation(direction):
	...
	return (
		...
		isBB(direction) and
		not isDoji() and
		isPosInProfit(direction) and
		not isPosInDirection(direction)
	)
