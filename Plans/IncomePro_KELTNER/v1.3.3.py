...
def confirmation(trigger, entry_type, reverse=False):
	...
	if entry_type == EntryType.PreTEntry or entry_type == EntryType.TEntry or entry_type == EntryType.CtEntry:
		cancel = False
		if pending_entry and pending_entry.direction != trigger.direction:
			pending_entry = None

		for pos in utils.positions:
			if pos.direction == Constants.BUY and trigger.direction == Direction.LONG and pos.data['type'] != EntryType.CtEntry.value:
				return False
			elif pos.direction == Constants.SELL and trigger.direction == Direction.SHORT and pos.data['type'] != EntryType.CtEntry.value:
				return False
