...
def init(utilities):
	...
	global donch, donch_exit

	donch = utils.DONCH(VARIABLES['donch'])
	donch_exit = utils.DONCH_CMC(4)
...
def handleEntries():
	...
	if pending_entry:
		...
		if pending_entry.is_exit:
			utils.log('handleEntries', "Attempting position close: all".format(pending_entry.direction))
			handleCloseAllPositions()
		elif isOppDirectionPositionExists(pending_entry.direction):
...
def entrySetup(trigger):
	...
	if trigger:
		...
		if isPositionInDirection(trigger.direction):
			if isDonchExitRet(trigger.direction) and isPosInProfit():
				confirmation(trigger, is_exit=True)
		else:
			trigger.can_exit = False
...
def canExit(direction):
	if direction == Direction.LONG:
		return isDonchExitExc(direction)
	else:
		return isDonchExitExc(direction)
...
def isDonchExitRet(direction, reverse=False):
	vals = donch_exit.get(utils, chart, 0, 2)
	if reverse:
		if direction == Direction.LONG:
			return vals[1][1] > vals[0][1]
		else:
...
def isDonchExitExc(direction, reverse=False):
	vals = donch_exit.get(utils, chart, 0, 2)
...
def confirmation(trigger, is_exit=False, reverse=False):
	''' confirm entry '''

	global pending_entry

	if pending_entry and is_exit:
		return
	utils.log("confirmation", '{0} {1}'.format(trigger.direction, reverse))
		
	pending_entry = trigger
	pending_entry.is_exit = is_exit
	return True
