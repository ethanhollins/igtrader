...
def handleStopAndReverse(entry):
	...
	if bank:
		...
		risk = VARIABLES['risk'] if entry.entry_type == EntryType.REGULAR else VARIABLES['ad_risk']
		
		pos = utils.stopAndReverse(
			VARIABLES['PRODUCT'], 
			utils.getLotsize(bank, risk, VARIABLES['stoprange']), 
...
def adEntrySetup(trigger):
	...
	if trigger and isPositionInDirection(trigger.direction):
		...
		if trigger.ad_entry_state == AdEntryState.ONE:
			if isBB(trigger.direction, reverse=True):
...
def adEntryConfirmation(trigger):
	...
	if trigger.direction == Direction.LONG:
		return close > trigger.ad_entry_line and time_state.value == TimeState.TRADING.value
	else:
		return close < trigger.ad_entry_line and time_state.value == TimeState.TRADING.value
