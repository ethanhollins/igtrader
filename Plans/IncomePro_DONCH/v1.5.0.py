...
def handleStopAndReverse(entry):
	...
	if bank:
		...
		if pos:
			pos.data['type'] = entry.entry_type.value
...
def runSequence():
	...
	if time_state != TimeState.STOP:
		entered = entrySetup(long_trigger)
		entered = entrySetup(short_trigger)

		if entered:
			resetOppositeTrigger()
		else:
...
def adEntrySetup(trigger):
	...
	if trigger and isPositionInDirection(trigger.direction):
		...
		if trigger.ad_entry_state == AdEntryState.ONE:
			if adEntryConfirmation(trigger.direction):
				trigger.ad_entry_line = getAdEntryLine(trigger.direction)
			...
			if adBreakevenConfirmation(trigger):
				trigger.ad_entry_state = AdEntryState.COMPLETE
				return setAdToBE()

def getAdEntryLine(direction):
...
def adEntryConfirmation(direction):
	if utils.plan_state.value in (1,4):
		utils.log('adEntryConfirmation', 'Additional Entry Conf: {0} {1}'.format(
			isBB(direction, reverse=True), time_state.value == TimeState.TRADING.value
		))

	return isBB(direction, reverse=True) and time_state.value == TimeState.TRADING.value

def adBreakevenConfirmation(trigger):
	close = chart.getCurrentBidOHLC(utils)[3]

	if trigger.direction == Direction.LONG:
		return close > trigger.ad_entry_line
	else:
		return close < trigger.ad_entry_line

def setAdToBE():
	for pos in utils.positions:
		if not pos.isBreakeven() and pos.data['type'] == EntryType.ADDITIONAL.value:
			pos.breakeven()

def resetOppositeTrigger():
