...
def entrySetup(trigger):
	...
	if trigger:
		if entryConfirmation(trigger) and not isPositionInDirection(trigger.direction):
			return confirmation(trigger)

def entryConfirmation(trigger):
	if isUpperDonchConf(trigger.direction):
		trigger.upper_conf = True
	elif isUpperDonchConf(trigger.direction, reverse=True):
		trigger.upper_conf = False

	if isLowerDonchConf(trigger.direction):
		trigger.lower_conf = True
	elif isLowerDonchConf(trigger.direction, reverse=True):
		trigger.lower_conf = False
	...
	return (
		trigger.upper_conf and
		trigger.lower_conf
	)

def isUpperDonchConf(direction, reverse=False):
	if reverse:
		if direction == Direction.LONG:
			return isDonchRet(direction)
		else:
			return isDonchExc(direction, reverse=True)
	else:
		if direction == Direction.LONG:
			return isDonchExc(direction)
		else:
			return isDonchRet(direction, reverse=True)

def isLowerDonchConf(direction, reverse=False):
	if reverse:
		if direction == Direction.LONG:
			return isDonchExc(direction, reverse=True)
		else:
