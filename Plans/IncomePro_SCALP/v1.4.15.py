...
def getPivotLines():
	...
	for i in range(len(ohlc_vals)-1, -1, -1):
		...
		if not Direction.LONG in obos_hist:
			...
			if not is_long_start and stridx <= 50:
				is_long_start = True

			if is_long_start and stridx >= VARIABLES['overbought']:
		...
		if not Direction.SHORT in obos_hist:
			...
			if not is_short_start and stridx >= 50:
				is_short_start = True

			if is_short_start and stridx <= VARIABLES['oversold']:
