...
def getPivotLines():
	...
	is_long_start = False
	is_short_start = False

	if rsi_vals[-1] >= VARIABLES['overbought_2']:
		trigger.pivot_line = ohlc_vals[-1][3]
		return
	elif rsi_vals[-1] <= VARIABLES['oversold_2']:
		trigger.pivot_line = ohlc_vals[-1][3]
		return
	...
	for i in range(len(ohlc_vals)-1, -1, -1):
		...
		if not Direction.LONG in obos_hist:
		...
		if not Direction.SHORT in obos_hist:
		...
		if len(obos_hist) > 0:
			if curr_obos == Direction.LONG:
				...
				if stridx <= 50:
					curr_obos = None
					trigger.obos_pivots[0] = curr_ex_long

					if ohlc_vals[-1][3] >= curr_ex_long:
						trigger.pivot_line = ohlc_vals[-1][3]
						return
				...
				if stridx >= 50:
					curr_obos = None
					trigger.obos_pivots[1] = curr_ex_short

					if ohlc_vals[-1][3] <= curr_ex_short:
						trigger.pivot_line = ohlc_vals[-1][3]
						return
...
def setPivotLine():
	if not trigger.pivot_line:
		...
		if high > trigger.obos_pivots[0] and stridx >= VARIABLES['overbought_2']:
			trigger.pivot_line = close if close < trigger.obos_pivots[0] else trigger.obos_pivots[0]
			return True

		elif low < trigger.obos_pivots[1] and stridx <= VARIABLES['oversold_2']:
			trigger.pivot_line = close if close > trigger.obos_pivots[0] else trigger.obos_pivots[0]
			return True
...
def confirmation(trigger, entry_type, reverse=False):
	...
	if not trigger.pivot_line or time_state == TimeState.STOP or isPosInDir(pending_entry.direction):
		pending_entry = None
		return False
