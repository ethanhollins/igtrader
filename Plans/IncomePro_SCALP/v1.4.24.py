...
def getPivotLines():
	...
	for i in range(len(ohlc_vals)-1, -1, -1):
		...
		long_ex = close if close > long_ex or long_ex == 0 else long_ex
		short_ex = close if close < short_ex or short_ex == 0 else short_ex

		if ch_idx >= 100:
			if direction == Direction.SHORT:
				trigger.pivot_data[0] = direction
				trigger.pivot_data[1] = short_ex
				return
			elif direction == None:
...
def setPivotLine():
	if not trigger.pivot_line:
		...
		if trigger.pivot_data[0] == Direction.LONG:
			close = close if close > trigger.pivot_data[1] else trigger.pivot_data[1]
			if chidx < -100:
				trigger.pivot_line = trigger.pivot_data[1]
				return True
		else:
			close = close if close < trigger.pivot_data[1] else trigger.pivot_data[1]
			if chidx > 100:
