...
def onNewBar(chart):
	...
	ohlc_ask = chart.getCurrentAskOHLC(utils)
	ohlc = np.concatenate((ohlc_ask, ohlc))
	ts = utils.convertDatetimeToTimestamp(datetime.datetime(2018,1,1))
	off = chart.getTsOffset(ts)
	# data = np.array(chart.getAllBidOHLC(utils)[off:], dtype=np.float32)
	data = np.array(chart.getBidOHLC(utils, 0, 500), dtype=np.float32)

	# if not data.shape[0] > 3:
	# 	return
	# print(getInputs(data))
	...
	if c_dir == Constants.BUY:
		if out[0] > threshold:
			...
			print('SELL (S&R) {} - {} | {:.2f}'.format(chart.c_ts, ohlc, utils.getTotalProfit() * 55))
			if bank:
		...
		if out[1] > threshold:
			...
			print('BUY (S&R) {} - {} | {:.2f}'.format(chart.c_ts, ohlc, utils.getTotalProfit() * 55))
			if bank:
				pos = utils.stopAndReverse(
					VARIABLES['PRODUCT'], 
					utils.getLotsize(bank, VARIABLES['risk'], VARIABLES['stoprange']), 
		...
		if out[0] > out[1]:
			if out[0] > threshold:
				...
				print('SELL (REG) {} - {} | {:.2f}'.format(chart.c_ts, ohlc, utils.getTotalProfit() * 55))
				if bank:
					pos = utils.sell(
						VARIABLES['PRODUCT'], 
						utils.getLotsize(bank, VARIABLES['risk'], VARIABLES['stoprange']), 
			...
			if out[1] > threshold:
				...
				print('BUY (REG) {} - {} | {:.2f}'.format(chart.c_ts, ohlc, utils.getTotalProfit() * 55))
				if bank:
					pos = utils.buy(
						VARIABLES['PRODUCT'], 
						utils.getLotsize(bank, VARIABLES['risk'], VARIABLES['stoprange']), 
