...
class LightstreamerSubscription(object):
    ...
    def notifyupdate(self, item_line):
        ...
        for on_item_update in self._listeners:
            t = Thread(target=on_item_update, args=(item_info,))
            t.start()
...
class LightstreamerClient(object):
    ...
    def connect(self):
        ...
        # print("Opening a new session to <{0}>".format(self._lightstreamer_url.geturl()))
    ...
    def bind(self, control_url='', session_id=''):
        """Replace a completely consumed connection in listening for an active Session."""

        if control_url and session_id:
            self._set_control_link_url(control_url)
            self._stream_connection = self._call(self._control_url, BIND_URL_PATH, {"LS_session": session_id})
        else:
    ...
    def _handle_stream(self, stream_line):
        if stream_line == OK_CMD:
            print("Successfully connected to <{0}>".format(self._lightstreamer_url.geturl()))
    ...
    def disconnect(self):
        ...
        if self._stream_connection is not None:
            print("Closing session to <{0}>".format(self._lightstreamer_url.geturl()))
